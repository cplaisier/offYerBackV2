...
if not os.path.exists('output/c1_postProc.pkl'):
    ...
    ratioFile = open('../genesExpMatrix_preprocessed.tsv','r')
    ...
    if not os.path.exists('output/biclusterEigengenes.csv'):
        eigengeneProc = Popen('R --no-save < getEigengene.R', shell=True, stdout=PIPE, stderr=PIPE)
        eigengeneProc.communicate()
    ...
    tfName2entrezId = {}
    inFile = open('TF/humanTFs_All.csv','r')
    inFile.readline() # Get rid of header
    while 1:
        line = inFile.readline()
        if not line:
    ...
    tfFamilies = {}
    inFile = open('TF/tfFamilies.csv','r')
    inFile.readline() # Get rid of header
    while 1:
        line = inFile.readline()
        if not line:
    ...
    # Get microarray data
    expData = {}
    inFile = open('expression/GSE35673_normalized_ilmn_id_allGenes.tsv','r')
                ...
                for factor in factors:
                    ...
                    if factor['factor'] in expData.keys():
                        ...
                        if not factor['factor'] in compared:
                                ...
                                ### alomana 23 oct 2014
                                variable1=[eigengene[i] for i in eigengene.keys()]
                                variable2=[expData[factor['factor']][i.replace('ratios.','')] for i in eigengene.keys()]
                                cor1 = correlation(variable1,variable2)
                                ###
                                
                                if corMax==[] or abs(cor1[0])>abs(corMax[0]):
                                    corMax = cor1
                    if not corMax==[]:
                        pssm.addCorrelatedMatch(factor['factor'],corMax[0],corMax[1])
    print 'Done.\n'
    ...
    for bicluster in c1.getBiclusters():
                ...
                for factor2 in factors[factor1]:
                    ...
                    if factor2 in expData:
                        ...
                        if not factor2 in compared:
                            ...
                            ### alomana 23 oct 2014
                            variable1=[eigengene[i] for i in eigengene.keys()]
                            variable2=[expData[factor2][i.replace('ratios.','')] for i in eigengene.keys()]
                            cor1 = correlation(variable1,variable2)
                            ###
        ...
        if not correlatedFactor==[]:
            #print correlatedFactor
    ...
    if not os.path.exists('output/m2m.pkl'):
        ...
        m2m = miRvestigator(pssms.values(),seqs3pUTR,seedModel=[6,7,8],minor=True,p5=True,p3=True,wobble=False,wobbleCut=0.25,baseDir='output',species='hsa')
    ...
    for line in inLines:
        ...
        b1.addAttribute(key='resid.norm.perm.p',value=str(splitUp[10]))
        b1.addAttribute(key='pc1.perm.p',value=str(splitUp[13]))
    inFile.close()
    print 'Done.\n'
    ...
    for meme1 in ['meme_motif1','meme_motif2']:
        if not upstreamMotifs[meme1]==None:
            ...
            if not pssm1.getMatches()==None:
                ...
                tmp = pssm1.getCorrelatedMatches()
                if not tmp==None:
                    for match1 in tmp:
            ...
            writeMe += [str(pssm1.getEValue()), # E-value
                        ...
                        correlatedMatches, # Correlated matches using expression data vs. the bicluster eigengene
                        originalExpanded, # Original factor or expanded
                        minCorrelated] # Minimum correlation
        else:
            writeMe += ['NA', # E-value
                        ...
                        'NA', # Correlated matches using expression data vs. the bicluster eigengene
                        'NA', # Original or expanded match
                        'NA'] # Minimum correlation
    #   - WEEDER motifs
    for weeder1 in ['weeder_motif1','weeder_motif2']:
        if not upstreamMotifs[weeder1]==None:
            ...
            writeMe += ['NA', # E-value
                        ...
                        'NA', # Matches to the motif from TransFac and Jaspar
                        'NA', # Expanded matches using TFClass
                        'NA', # Correlated matches using expression data vs. bicluster eigengenes
                        'NA',
                        'NA']
    ...
    for association in ['tfbs_db']:
        ...
        if not a1['tf']=='':
            ...
            if len(correlatedMatches)==0:
                correlatedMatches = 'NA'
            writeMe += [str(a1['tf']).replace(';',' '), str(a1['percentTargets']).replace(';',' '), str(a1['pValue']), expandedMatches, correlatedMatches,originalExpanded,minCorrelated]
        else:
...
header = ['Bicluster', 'Genes', 'Conditions', 'Norm. Residual', 'Norm. Residual Perm. P-Value', 'Var. Exp. First PC', 'Var. Exp. First PC Perm. P-Value', 'MEME Motif1 E-Value', 'Up.MEME Motif1 Consensus', 'Up.MEME Motif1 Matches','Up.MEME Motif1 Expanded Matches', 'Up.MEME Motif1 Correlated Matches', 'Up.MEME Motif1 Original/Expanded', 'Up.MEME Motif1 Minimum Correlated', 'Up.MEME Motif2 E-Value', 'Up.MEME Motif2 Consensus', 'Up.MEME Motif2 Matches', 'Up.MEME Motif2 Expanded Matches', 'Up.MEME Motif2 Correlated Matches', 'Up.MEME Motif2 Original/Expanded', 'Up.MEME Motif2 Minimum Correlated', 'Up.WEEDER Motif1 Score', 'Up.WEEDER Motif1 Consensus', 'Up.WEEDER Motif1 Matches', 'Up.WEEDER Motif1 Expanded Matches', 'Up.WEEDER Motif1 Correlated Matches', 'Up.WEEDER Motif1 Original/Expanded', 'Up.WEEDER Motif1 Minimum Correlated', 'Up.WEEDER Motif2 Score', 'Up.WEEDER Motif2 Consensus', 'Up.WEEDER Motif2 Matches', 'Up.WEEDR Motif2 Expanded Matches', 'Up.WEEDER Motif2 Correlated Matches', 'Up.WEEDER Motif2 Original/Expanded', 'Up.WEEDER Motif2 Minimum Correlated', 'TFBS_DB.TFs', 'TFBS_DB.percTargets', 'TFBS_DB.pValue', 'TFBS_DB.Exapnded Matches', 'TFBS_DB.Correlated Matches', 'TFBS_DB.Original/Expanded','TFBS_DB.Minimum Correlated', '3pUTR.WEEDER Motif1 E-Value', '3pUTR.WEEDER Motif1 Consensus', '3pUTR.WEEDER Motif1 Matches', '3pUTR.WEEDER Motif1 Model', '3pUTR.WEEDER Motif2 E-Value', '3pUTR.WEEDER Motif2 Consensus', '3pUTR.WEEDER Motif2 Matches', '3pUTR.WEEDER Motif2 Model', '3pUTR_pita.miRNAs', '3pUTR_pita.percTargets', '3pUTR_pita.pValue', '3pUTR_targetScan.miRNAs', '3pUTR_targetScan.percTargets', '3pUTR_targetScan.pValue', 'GO_Term_BP'] #+ [i.strip() for i in hallmarksOfCancer]
