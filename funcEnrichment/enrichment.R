...
#convertMe = as.list(illuminaHumanv3ENTREZID)
...
for(j in 1:length(d1[,1])) {
    #biclustMembership[[j]] = c(na.omit(as.character(convertMe[strsplit(as.character(d1[j,]),split=' ')[[1]][-1]])))
...
xx = names(as.list(illuminaHumanv3GO))
#xx <- annFUN.org("BP", mapping = "illuminaHumanv3.db", ID = "entrez") # alomana 23 oct 2014
geneNames <- intersect(unique(unlist(biclustMembership)), unique(xx))
...
GOdata.BP <- new("topGOdata", ontology='BP', allGenes = geneList, annot = annFUN.db, affyLib = 'illuminaHumanv3.db')
...
# Make Biological Process GOData object
#GOdata.BP <- new("topGOdata", ontology='BP', allGenes = geneList, annot = annFUN.org, mapping = 'illuminaHumanv3.db', ID = 'entrez')
#m1.BP <- matrix(nrow = length(GOdata.BP@graph@nodes), ncol = length(biclustMembership), dimnames = list(GOdata.BP@graph@nodes, 1:length(biclustMembership)))
#m2.BP <- matrix(nrow = length(biclustMembership), ncol=2, dimnames = list(1:length(biclustMembership),c('Top10.Terms.BP','BH.sig.GO.Ids.BP')))
